...
const groups = [
  {
    ...
    items: [
      ...
      { to: "/admin/instructor-overtime", icon: Clock3, label: "Instructor Overtime" },
    ],
  },
];
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent("admin:sidebar-collapsed", { detail: { collapsed } }));
  }, [collapsed]);
  ...
  return (
    <aside className={cn("fixed inset-y-0 left-0 z-40 hidden md:flex flex-col border-r bg-background transition-[width] duration-200 ease-out", widthClass)}>
      {/* Header */}
      <div className={cn("flex items-center gap-2 px-3 py-3 border-b")}>
        ...
        {!collapsed && (
          <div className="min-w-0">
            <div className="text-sm font-semibold leading-tight">HR Admin Panel</div>
      ...
      <div className="border-t p-3">
        <div className={cn("flex items-center gap-3 mb-3", collapsed && "justify-center")}>
          ...
          {!collapsed && <Settings className="ml-auto h-4 w-4 text-muted-foreground" />}
        </div>
