    ...
    try {
      ...
      const infoRows = [
        ...
        { label: "Designation", value: user.designation || "-" },
        {
          label: "Branch/City",
          value: user.branch
            ? `${user.branch}${user.city ? `, ${user.city}` : ""}`
            : user.city || "-",
        },
      ];
      ...
      const totalsKm = verifiedItems.reduce(
      ...
      const buildPdf = (densityIndex) => {
        const layout =
          LAYOUT_STEPS[Math.min(densityIndex, LAYOUT_STEPS.length - 1)];
        ...
        pdf.text(
          headerMonth,
          pageWidth - margin - pdf.getTextWidth(headerMonth),
          headerHeight - 6
        );
        ...
        const colDesc = Math.max(
          40,
          contentWidth - (colSr + colKm + colRate + colAmount + colStatus)
        );
        ...
        const footRow = verifiedItems.length
          ? [
              [
                "",
                "Verified totals",
                totalsKm.toLocaleString(),
                "",
                totalsAmount.toLocaleString(),
                `${verifiedItems.length}/${allItems.length} verified`,
              ],
            ]
          : undefined;
        ...
        const metaTopLimit = signatureTop - 8;
        ...
        if (metaCursorY <= metaTopLimit) {
          ...
          pdf.text(
            "Totals reflect only verified line items.",
            margin,
            metaCursorY
          );
          pdf.setTextColor(0, 0, 0);
        ...
        if (metaCursorY <= metaTopLimit) {
          ...
          pdf.text(
            created,
            pageWidth - margin - pdf.getTextWidth(created),
            metaCursorY
          );
          metaCursorY += 3.5;
        }
        ...
        if (result.remarks && metaCursorY <= metaTopLimit) {
          pdf.setFontSize(layout.subFont);
          pdf.text(`Remarks: ${result.remarks}`, margin, metaCursorY, {
        ...
        pdf.text("Approved / Checked by:", margin, signatureTop);
        pdf.line(margin, signatureTop + 1.3, margin + 58, signatureTop + 1.3);

        const sigX = pageWidth - margin - 58;
        pdf.text("Signature:", sigX, signatureTop);
        pdf.line(sigX, signatureTop + 1.3, sigX + 58, signatureTop + 1.3);
      ...
      const finalizePdf = (densityIndex) => {
        ...
        if (
          candidate.getNumberOfPages() > 1 &&
          densityIndex < LAYOUT_STEPS.length - 1
        ) {
      ...
      if (pdf.getNumberOfPages() > 1) {
        toast.warning("Report still spans multiple pages.", {
          description: "Location has more rows than can fit on one sheet.",
        });
      }

      pdf.save(
        `FuelRequisition_${monthLabel || filters.month}_${
          result.year || filters.year
        }_${displayName}.pdf`
      );
    } catch (err) {
        ...
        <CardContent>
            ...
            <div className="space-y-1">
              ...
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={
                      loadingUsers ? "Loading..." : "Select employee"
                    }
                  />
                </SelectTrigger>
          ...
          {result ? (
            ...
            <div className="text-sm text-muted-foreground">
              select a user (only those who have requisitions are listed) and
              pick month/year.
            </div>
          )}
        </CardContent>
